...
export const getOrCreateHostingConfig = async (): Promise<HostingConfig | null> => {
    ...
    try {
        ...
        await puter.kv.set(HOSTING_CONFIG_KEY, { subdomain });
...
export const uploadImageToHosting = async ({hosting, url, projectId, label}: StoreHostedImageParams): Promise<HostedAsset | null> => {
    ...
    try {
        ...
        const hostedUril = getHostedUrl({ subdomain: hosting.subdomain }, filePath);

        return hostedUril ? { url: hostedUril } : null;
    } catch (error) {
        console.warn(error);
        return null;
    }
